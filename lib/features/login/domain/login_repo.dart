...
class LoginRepo {
  ...
  Future<CommonResponse> checkUserAvailability(EmailBody body) async {
    if (kDebugMode) {
      print(UrlUtils.checkIfUserExists);
    }

    try{
      Response response = await DioClient().dio.post(UrlUtils.checkIfUserExists, data: body.toJson(),
        // options: Options(
        //     headers: {
  ...
  Future<CommonResponse> requestOtp(EmailBody body) async {
    if (kDebugMode) {
      print(UrlUtils.sendOtpToEmail);
    }

    try{
      Response response = await DioClient().dio.post(UrlUtils.sendOtpToEmail, data: body.toJson(),
        // options: Options(
        //     headers: {
        //       'Content-Type':'application/json',
        //       'Authorization': '${AppConstantConfig.bearer}$bearerToken',
